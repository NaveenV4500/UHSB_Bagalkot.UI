...
namespace UHSB_MasterService.Common
{
    public static class FileUploadCheck
    {
        ...
        private enum DocumentFileExtension
        {
            none = 0,
            DOC = 1,
            DOCX = 2
        ...
        public static bool isValidFile(byte[] bytFile, FileType flType, String FileContentType)
        {
            ...
            {
                isvalid = isValidDocumentFile(bytFile, FileContentType);
            }
        ...
        public static bool isValidDocumentFile(byte[] bytFile, String FileContentType)
        {
            byte[] chkBytedoc = { 208, 207, 17, 224, 161, 177, 26, 225 };
            byte[] chkBytedocx = { 80, 75, 3, 4 };
            bool isvalid = false;

            DocumentFileExtension docfileExtn = DocumentFileExtension.none;
            if (FileContentType.Contains("application/msword"))
            {
                docfileExtn = DocumentFileExtension.DOC;
            }
            else if (FileContentType.Contains("officedocument") || FileContentType.Contains("application/octet-stream"))
            {
                docfileExtn = DocumentFileExtension.DOCX;
            }

            if (docfileExtn == DocumentFileExtension.DOC)
            {
                if (bytFile.Length >= 4)
                {
                    int j = 0;
            ...
            if (docfileExtn == DocumentFileExtension.DOCX)
            {
                if (bytFile.Length >= 4)
                {
                    int j = 0;
