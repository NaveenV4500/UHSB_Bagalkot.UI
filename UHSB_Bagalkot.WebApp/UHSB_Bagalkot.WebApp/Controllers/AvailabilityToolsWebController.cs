...
namespace UHSB_Bagalkot.WebApp.Controllers
{
    public class AvailabilityToolsWebController : Controller
    {
        ...
        [HttpPost]
        public IActionResult CentersAvailabilityDetails(AvailabilityToolsDetailsVM model)
        {
            if (model == null)
            {
            }   
        ...
        public async Task<IActionResult> GetGridContentV1(int currentPage = 1, int pageSize = 10, GridEnum.UserMasterColumns orderBy = GridEnum.UserMasterColumns.UserName, bool isDescending = false, string filterDetails = null, string externalFilter = null)
        {
            try
            {
                ...
                // Make GET request
                var response = await _httpClient.GetAsync($"{_apiSettings.Base_Url}/AvailabilityTools/getgridcontentavailabilitytools?{queryString}");
                ...
                var result = JsonConvert.DeserializeObject<ApiResponse<GenericGridModel<AvailabilityToolsDetailsVM>>>(content);
