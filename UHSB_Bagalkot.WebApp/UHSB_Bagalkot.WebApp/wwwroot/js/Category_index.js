$(document).ready(function () {
    ...
    window.GridFilterViewUrl = "Category/GetSearchView";
    window.orderByColumnId = "0";
    window.isDescendingFilter = false;

    var pageSizeKey = 'pagesize_Category';
    ...
    window.init = function () {
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: 'json',
            url: GetRootPath(window.virtualPath) + '/Category/GetGridContentV1',
            data: {
            ...
            success: function (response, successStatusText, responseJqXhrObject) {
                ...
                if (response.data.itemDetails && response.data.itemDetails.length) {
                    ...
                    $.each(response.data.itemDetails, function (index, item) {
                        ...
                        //}
                        //if (!response.CanEdit && !response.CanDelete) {
                        //    view_span = '<span id=' + item.categoryId + ' class="btn-link view" style="cursor:pointer" >View</span>';
                        //    rowContent = "<td class='text-center'>" + view_span + "</td>";
                        //}
                        var fileLink = item.imageUrl
                        ...
                        rowContent += '<td class="text-center">' + actionCell + '</td>' +
                            '<td class="text-center">' + (parseInt((currentPage - 1) * pageSize) + parseInt(index + 1)) + '</td>' +
                            '<td>' + BindValueToGrid(item.name, false, false) + '</td>';
                                    //'<td>' + BindValueToGrid(item.imageUrl, false, false) + '</td>';
    ...
    $('body').on('click', 'span.edit,span.view', function (e) {
        ...
        var selectedCategoryDetails = $.grep(gridJson.ItemDetails, function (e) {
        ...
        var selectedCategory = selectedCategoryDetails[0];
        // ;
        if (selectedCategory.RoleTypeId == 4 || selectedCategory.RoleTypeId == 6 || selectedCategory.RoleTypeId == 7) {
            $('#basebranchhide').hide();
        }
        else {
            $('#basebranchhide').show();
        ...
        $(editBindId + "UserId").val(selectedCategory.UserId);
        $(editBindId + "UserName").val(selectedCategory.UserName);
        $(editBindId + "FullName").val(selectedCategory.FullName);
        $(editBindId + "RoleType").val(selectedCategory.RoleTypeId);
        $(editBindId + "EmpId").val(selectedCategory.EmpId);
        $(editBindId + "Email").val(selectedCategory.Email);
        $(editBindId + "MobileNo").val(selectedCategory.MobileNo);
        $(editBindId + "BaseBranchId").val(selectedCategory.BaseBranchId);
        $(editBindId + "IsActive").prop('checked', (selectedCategory.IsActive));
    ...
    $('body').on('click', 'span.delete', function (e) {
        ...
        if (flag) {
            ...
            $.ajax({
                ...
                url: GetRootPath(window.virtualPath) + 'Category/UserDelete',
    ...
    $('#addNew').on('click', function (e) {
        ...
        ResetForm("#form1");
        $("#form1").find("input:text").val("");
        url = GetRootPath(window.virtualPath) + 'Category/Register';
        window.location.href = url;
        //$.get("~/Category/Register", null, function (data) {
        //    alert("hi");
        //});
        scrollTop();
        mode_edit = false
    ...
    $('body').on('click', '.resetpassword', function () {
        ...
        $("#form2 #UserName").val(selectedCategory.UserName);
    ...
    //    if (mode_edit) {
    //        ;
    //        var url = $('#rootpath').text() + "Category/Edit";
    ...
    $('#btn_submit').click(function () {
        ...
        var form = $('#form1');
        if (mode_edit) {
            var url = GetRootPath(window.virtualPath) + "Category/Edit";
            form.attr('action', url);
        }
        else {
            var url = GetRootPath(window.virtualPath) + "Category/Register";
            form.attr('action', url);
        }
    })
